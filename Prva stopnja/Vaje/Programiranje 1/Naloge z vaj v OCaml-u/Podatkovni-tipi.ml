...
(*
My own fuckery:
Let it_resolves = function
  | true -> true
  | false -> false

Let in_response = function
  | true -> false
  | false -> true
...
let dollar_to_euro dollar =
  match dollar with
  | Dollar v -> Euro (0.861 *. v)
  
  let euro_to_dollar euro =
    match euro with
    | Euro v -> Dollar (1.161 *. v)
...
let to_pound c =
  ...
  | Yen v -> Pound (0.007 *. v)
  | Pound v -> Pound v
  | Krona v -> Pound (0.086 *. v)
  | Franc v -> Pound (0.78 *. v)
...
let rec intbool_reverse ib_list = 
  let rec reverse_aux (acc : intbool_list) = function
    | Empty -> acc
    | Int_val (i, ib_tail) -> reverse_aux (Int_val(i, acc)) ib_tail
    | Bool_val (b, ib_tail) -> reverse_aux (Bool_val(b, acc)) ib_tail
  in reverse_aux Empty ib_list
...
let rec intbool_separate ib_list = 
  let rec separate' (int_acc : intbool_list) (bool_acc : intbool_list) = function
    | Empty -> [int_acc; bool_acc]
    | Int_val(i, ib_tail) -> separate' Int_val(i, int_acc) bool_acc ib_tail 
    | Bool_val(b, ib_tail) -> separate' int_acc Bool_val(b, bool_acc) ib_tail 
  in separate' Empty Empty ib_list
